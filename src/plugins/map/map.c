		   ...
		   u8 psid_length, u32 * map_domain_index, u16 mtu, u8 flags)
{
  ...
  if (flags & MAP_DOMAIN_TRANSLATION)
	...
	{
	  clib_warning ("MAP-T only supports prefix lengths 64 and 96.");
	  return -1;
	}
    }
  else
			    ...
			    vlib_cli_command_t * cmd)
{
  ...
  if (map_domain_index == ~0)
    {
    /* *INDENT-OFF* */
    pool_foreach(d, mm->domains, ({vlib_cli_output(vm, "%U",
	    format_map_domain, d, counters);}));
    /* *INDENT-ON* */
    }
  else
    {
...
int
map_if_enable_disable (bool is_enable, u32 sw_if_index, u8 is_translation)
{
  if (is_translation == 0)
    {
...
map_init (vlib_main_t * vm)
{
  ...
  mm->domain_counters[MAP_DOMAIN_COUNTER_RX].stat_segment_name = "/map/rx";
  mm->domain_counters[MAP_DOMAIN_COUNTER_TX].stat_segment_name = "/map/tx";
